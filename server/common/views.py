...
class DashboardAnalyticsView(APIView):
    def get(self, request, format=None):
        stock_value = Stock.objects.aggregate(
            value=Coalesce(Sum(F("quantity") * F("unit_purchase_price")), Value(0.0), output_field=FloatField())
        )["value"]
        potential_sales_value = Stock.objects.aggregate(
            value=Coalesce(Sum(F("quantity") * F("unit_selling_price")), Value(0.0), output_field=FloatField())
        )["value"]
        ...
        low_stock = list(
            ...
            .order_by("quantity")[:8]
            .values("id", "quantity", "unit_selling_price", product_name=F("product__name"))
        )

        top_products = list(
            Stock.objects.select_related("product")
            .order_by("-quantity")[:8]
            .values("id", "quantity", product_name=F("product__name"))
        )
        ...
        purchases_by_day = list(
            ...
            .values("day")
            .annotate(spent=Coalesce(Sum("total"), Value(0.0), output_field=FloatField()))
            .order_by("day")[:14]
        )
        recent_sales = list(
        ...
        recent_purchases = list(
            ...
            .values("bill_no", "total", "paid_amount", "due_amount", "created_at", supplier_name=F("supplier__name"))
        )
