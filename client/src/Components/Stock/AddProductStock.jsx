...
const AddProductStock = () => {
  ...
  const updateProductStock = async()=>{
    const data = { ...stockInfo };
    data["quantity"] = parseInt(data.quantity);
    const response = await fetch(`${stockUrl}${productStockId}/`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
      });
  ...
  useEffect(() => {
    ...
    if (productStockId) {
      fetch(`${stockUrl}${productStockId}/`)
        .then((res) => res.json())
        .then((data) => setStockInfo(data));
    }
  }, [productUrl, productStockId, stockUrl]);
    ...
    <>
      ...
      <div className="container px-5 py-6 mx-auto">
        ...
        <div className="flex my-2 justify-center">
          <button
            type="button"
            onClick={productStockId? updateProductStock: addProudctStock}
